-- =============================================================================
-- Fix: 578 of 792 DAB CPFs are silently dropped by apply_payroll_staging().
--
-- Root cause: apply_payroll_staging() uses INNER JOIN with external_employees.
-- CPFs not in that table (but present in DAB payroll data) are lost.
-- external_employees has 873 rows but covers only ~214 of the 792 payroll CPFs.
--
-- Fix:
--   1. Add razao_social + tenant_id columns to payroll_verba_staging
--      (sync-verbas now populates both from each DAB row)
--   2. Rewrite apply_payroll_staging() with two-pass resolution:
--        Pass 1 (primary)  — INNER JOIN external_employees  (enriched: dept/pos/group)
--        Pass 2 (fallback) — JOIN companies by company name  (for unregistered CPFs)
--          Matches on: razao_social (normalized) OR tenant_id (UPPERCASE_UNDERSCORE)
-- =============================================================================

-- ─── 1. Add columns to staging ───────────────────────────────────────────────

ALTER TABLE public.payroll_verba_staging
  ADD COLUMN IF NOT EXISTS razao_social TEXT NOT NULL DEFAULT '',
  ADD COLUMN IF NOT EXISTS tenant_id    TEXT NOT NULL DEFAULT '';

-- ─── 2. Rewrite apply_payroll_staging() ──────────────────────────────────────
...
BEGIN
  ...
  WITH
    -- ── Pass 1: via external_employees (preferred — carries dept/position/group) ─
    from_employees AS (
      SELECT DISTINCT ON (
    ...
    -- ── Pass 2: via companies.name matching razao_social or tenant_id ─────────
    -- Only for CPFs that have no external_employees entry.
    from_company_name AS (
      ...
      JOIN public.companies c
        ON (
          -- Match by razao_social (case-insensitive, punctuation-stripped)
          (s.razao_social <> ''
            AND lower(REGEXP_REPLACE(trim(c.name), '[^a-zA-Z0-9]+', ' ', 'g'))
              = lower(REGEXP_REPLACE(trim(s.razao_social), '[^a-zA-Z0-9]+', ' ', 'g')))
          OR
          -- Match by tenant_id (UPPERCASE_UNDERSCORE — most reliable)
          (s.tenant_id <> ''
            AND UPPER(REGEXP_REPLACE(REGEXP_REPLACE(trim(c.name), '[^A-Za-z0-9 ]', '', 'g'), ' +', '_', 'g'))
              = s.tenant_id)
        )
      WHERE (p_ano IS NULL OR s.ano = p_ano)
        -- Only for CPFs not in external_employees
        AND NOT EXISTS (
          SELECT 1 FROM public.external_employees ee2
    ...
    -- ── Union: pass 1 wins over pass 2 for the same (cpf, tipo_verba, ano) ────
    all_resolved AS (
  ...
  DO UPDATE SET
    ...
    -- Preserve richer data from external_employees when re-running with pass 2
